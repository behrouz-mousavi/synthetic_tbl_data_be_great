...
def _pad(x, length: int, pad_value=2):
    """
...
#
def _pad_tokens(tokens, pad_value=2):
    """
...
class CategoricalStart(GReaTStart):
    ...
    def get_start_tokens(self, n_samples):
        ...
        start_tokens = self.tokenizer(start_text, padding='longest')["input_ids"]
        return start_tokens
...
class RandomStart(GReaTStart):
    ...
    def get_start_tokens(self, n_samples):
        ...
        #start_tokens = _pad_tokens(self.tokenizer(start_text)["input_ids"])
        start_tokens = self.tokenizer(start_text, padding='longest')["input_ids"]
        #start_tokens = _pad_tokens(self.tokenizer(start_text)["input_ids"], pad_value=self.tokenizer.pad_token_id)
        return start_tokens
